...
Yaxshi taqqoslash: E-IMZO'ni "Google orqali kirish" yoki to'lov tizimining webhook'i kabi tasavvur qiling. Siz kriptografiyani o'zingiz yozmaysiz — ishni E-IMZO'ga topshirasiz, keyin natijaning haqiqiyligini tekshirasiz. Xavfsizlik "so'rovni kim yubordi" degan savolga emas, **"imzo matematik jihatdan to'g'rimi"** degan savolga asoslanadi.

E-IMZO'ni **uch xil usulda** ulash mumkin. Har biri imzoni boshqacha yo'l bilan yaratadi:
...
> ⚠️ **Hammadan ko'p qiynaydigan narsa — hash algoritmi.** E-IMZO SHA-256 emas, **O'z DSt 1106** (bu — CryptoPro parametrli GOST R 34.11-94) ishlatadi. Agar algoritmni adashtirsangiz, hech qanday xato chiqmaydi — imzo shunchaki **jimgina verify'dan o'tmaydi**, va siz sababini soatlab qidirasiz. Shuning uchun [3-bo'lim](#3-hash--oz-dst-1106-eng-muhim-bolim) — eng muhimi.

---

## 2. Ishtirokchilar (qismlar)

Integratsiyaga kirishishdan oldin, kim nima ish qilishini bilib oling:
...
- **SiteID** — E-IMZO sizga beradigan identifikator (do'kon uchun "merchant ID"ga o'xshaydi). Mobile jarayonda siz SiteID uchun bitta **callback URL** registratsiya qilasiz.
- **PKCS#7** — imzoning "muhrlangan konverti". Uning ichida: kim imzolagan, uning sertifikati, va kriptografik imzoning o'zi bor. Ikki turi bor: *attached* (hujjat konvert ichiga solingan) va *detached* (faqat imzo, hujjatsiz).
...
Qaysi tilda bo'lmasin, GOST implementatsiyangizga u quyidagi natijalarni bermaguncha ishonmang. Bular — hammaga ochiq, e'lon qilingan **CryptoPro GOST R 34.11-94 test vektorlari** (O'z DSt 1106 bilan bir xil). Ularni unit-test qilib qo'ying:
...
Agar sizning `hash("")` natijangiz `981e5f3c…` bo'lsa — algoritm to'g'ri. Agar boshqacha bo'lsa (masalan, GOST'ning *test* to'plami `ce85b99c…` beradi), u E-IMZO bilan **hech qachon verify'dan o'tmaydi**.
...
Agar sizning arxitekturangiz server tomonda bo'lsa (hujjat backend'da tursa), hash'ni backend'da hisoblash eng oson yo'l: shunda ilovaga butun faylni emas, atigi 64 belgili `hashHex` string qaytarasiz — bu trafik uchun ham yengil.
...
| Endpoint | Nima qiladi |
|---|---|
| `POST /frontend/mobile/auth` | Mobile orqali **login**ni boshlaydi; imzolash uchun tasodifiy `challange` qaytaradi |
...
3. Tanlangan kalitni yuklaysiz, so'ng PKCS#7'ni **plugin ichida** yaratasiz. Plugin hujjatni O'z DSt 1106 bilan hashlaydi va imzolaydi. Natijada base64 formatdagi PKCS#7 (attached yoki detached) olasiz.
...
Telefonda sizning kodingiz boshqaradigan lokal plugin yo'q. Shuning uchun mobile jarayoni **server tomonda** quriladi: backend hash'ni tayyorlaydi, telefon E-IMZO ilovasiga deep-link orqali o'tib imzolaydi, va **E-IMZO'ning markaziy serveri tayyor PKCS#7'ni sizning callback URL'ingizga qaytaradi**.
...
3. **Ilova:** shu ma'lumotlardan deep-link quradi va uni ochadi (formati pastda, 6.2).
...
`qc` — bu bitta uzun string bo'lib, u `siteId`, `documentId`, `hashHex` va `crc32`ning ketma-ket ulanishidan hosil bo'ladi. Bu yerda `crc32 = CRC32(siteId + documentId + hashHex)` bo'lib, u stringning oxiriga qo'shiladi.

Backend allaqachon `hashHex` bergan bo'lsa, Dart'da (reference koddan):

```dart
String code = siteId + documentId + hashHex;      // hashHex — backend'dan keladi
code += Crc32.calcHex(code);                        // faqat CRC32 client tomonda (crc32.dart)
...
E-IMZO markaz PKCS#7'ni SiteID'ingizga registratsiya qilingan URL'ga **`application/x-www-form-urlencoded`** formatda `POST` qiladi. Maydon (field) nomlari quyidagicha — bularni biz **haqiqiy production callback'dan** ko'rib, tasdiqladik:

| Field nomi | Ma'nosi |
|---|---|
| `document_id` | siz bergan session id'si |
| **`pkcs7_b64`** | detached PKCS#7 imzo, base64 formatda — **e'tibor bering, `pkcs7` emas!** |
| `serial_number` | imzolovchi sertifikatining seriya raqami |
| `x_real_ip` | E-IMZO ko'rgan imzolovchining IP manzili |

> **Aynan shu bizni juda qattiq qiynadi.** Biz `pkcs7` degan field'ni o'qidik, u bo'sh chiqdi, va har bir callback `400` bilan rad etildi. Natijada session'lar hash ham, URL registratsiyasi ham to'g'ri bo'lsa-da, mangu "PENDING"da qolib ketaverdi. **Yechim: `pkcs7_b64` field'ini o'qing.** Umuman, birinchi haqiqiy callback kelganda butun `POST` body'sini log qiling (masalan, `request.getParameterMap().keySet()`) va E-IMZO aslida qanday nomlar yuborayotganini o'z ko'zingiz bilan ko'ring.
...
Session holatini saqlash uchun **Redis** juda qulay: status + TTL (muddat) + bitta bir martalik lock (takroriy callback ikki marta ishlab ketmasligi uchun). Session uchun alohida baza jadvali kerak emas; doimiy imzoni sizning oddiy verify+saqlash kodingiz yozadi.
...
2. **Kriptografik verify** — soxta yoki o'zgartirilgan PKCS#7 `verifyDetached`'dan o'tmaydi, va natijada **hech narsa saqlanmaydi**.
3. **PINFL tekshiruvi** — sertifikatdagi milliy ID kutilgan imzolovchiga mos kelishi shart.
...
- E-IMZO'dan **SiteID** oling (shartnoma asosida beriladi).
- O'sha SiteID uchun E-IMZO'ga o'zingizning **callback (UPLOAD) URL**ingizni registratsiya qildiring — E-IMZO markaz PKCS#7'ni aynan shu manzilga yuboradi. Bu registratsiya E-IMZO tomonda qilinadi, sizning kodingizda emas.
- e-imzo-server'ingizning mobile modulini o'sha `mobile.siteId` bilan sozlang va yoqing.
- Callback URL E-IMZO'dan yetib boradigan ochiq HTTPS manzil bo'lishi, va siz registratsiya qilgan manzil bilan **aynan bir xil** bo'lishi kerak.

Diagnostika uchun oddiy qoida: agar E-IMZO callback'i log'da umuman ko'rinmasa (`/upload` ga hech qanday so'rov kelmasa) — demak SiteID→URL registratsiyasi noto'g'ri yoki yo'q. Agar so'rov kelsa-yu `400` qaytsa — demak field nomida muammo (6.3).
...
|---|---|---|
| Imzo **hech qachon verify'dan o'tmaydi** | Noto'g'ri hash algoritmi (SHA-256, yoki GOST'ning *test* to'plami) | O'z DSt 1106 = CryptoPro GOST R 34.11-94 ishlating; [3.2](#32-har-qanday-implementatsiyani-shu-vektorlar-bilan-tekshiring) vektorlari bilan tekshiring |
| Faqat ba'zi hujjatlar verify bo'lmaydi | Xom baytlar o'rniga **base64 string** hashlangan | Hujjatning xom baytlarini hashlang |
| Mobile session **mangu PENDING**, log'da `/upload` yo'q | Callback URL registratsiya qilinmagan / noto'g'ri SiteID / mobile moduli o'chiq | Registratsiyani tekshiring ([8-bo'lim](#8-siteid-va-callback-url-registratsiyasi)); `/frontend/mobile/*` ni `404` uchun `curl` qiling |
...
| `/upload` `200` qaytaradi, lekin verify **FAILED** | `application/x-www-form-urlencoded`da base64 ichidagi `+` belgisi bo'sh joyga aylangan; yoki noto'g'ri hujjat baytlari | `pkcs7.replace(' ', '+')`; imzolash va verify aynan bir xil baytlar ustidan bo'lsin |
| `documentId` g'alati ko'rinadi (uzun UUID) | Siz uni o'zingiz yasayapsiz (stub/placeholder), `/frontend/mobile/sign` ni chaqirmasdan | Haqiqiy allocator'ni ishlating; haqiqiy id qisqa bo'ladi (masalan `A1B2C3D4`) |
...
**Eng foydali diagnostika usuli:** birinchi haqiqiy callback kelganda butun so'rovni (content type + barcha field nomlari + uzunliklari) log qiling. E-IMZO'ning haqiqiy field nomlari — asosiy haqiqat manbai; hujjatlar realdan ba'zan orqada qoladi.
...
*Bu qo'llanma haqiqiy, production'da ishlab turgan E-IMZO integratsiyasidan (web + mobile + backend) yozildi. Agar bu yerdagi biror field nomi yoki endpoint sizning jonli E-IMZO'ingizga to'g'ri kelmasa — jonli callback'ga ishoning va bu hujjatni yangilang: E-IMZO'ning real xatti-harakati eng ishonchli manba.*
